...
def z_bin_x_stat(array,bin_no): # array[:,(z,x)]
    if bin_no == False: # Useful for Abundance plots, unnecessary for GC plots
        array = array[array[:, 0].argsort()] # sort array by z vector
        bin_no = len(array)//500_000 # Determining bin no
        bins = np.linspace(0,bin_no*500_000,bin_no+1) # creating equally SIZED bins
        bins = np.digitize(np.linspace(0,len(array),len(array)),bins) # assigning bins based on index value
        array = np.hstack((np.array([bins]).T,array)) # array[:,(zbin,z,x)]
        u = np.array([np.mean(array[array[:,0]==i,2]) for i in range(1,bin_no+1)]) # mean x value in each bin
        s = np.array([np.std(array[array[:,0]==i,2]) for i in range(1,bin_no+1)]) # standard deviation of x in each bin
        m = np.array([np.median(array[array[:,0]==i,2]) for i in range(1,bin_no+1)])
        X = np.array([len(array[array[:,0]==i,2]) for i in range(1,bin_no+1)])
        u_z = np.array([np.mean(array[array[:,0]==i,1]) for i in range(1,bin_no+1)]) # mean z value in each bin
        return(np.vstack((u_z, u,s,m,X)).T)
    elif bin_no:
        bin_min = np.min(array[:,0])
        bin_max = np.max(array[:,0])
        bins = np.linspace(bin_min,bin_max,bin_no+1) # creating equally spaced bins of a set number from min(x) to max(x)
...
for i in range(2,8):
    genome = []
    for chrom in chrs:
        z_chr = np.load(var[1]+chrom+'.npy').astype('float32') # Abundance
        x_chr = np.load(var[i]+chrom+'.npy').astype('float32') # ChIP
        tss = np.load(var[8]+chrom+'.npy').astype('intc') # TSS
        locs = np.where((tss>0) & (z_chr>0) & (x_chr>0))
        del tss
        z_chr = z_chr[locs]
        x_chr = x_chr[locs]
        del locs
        z_chr = np.log(z_chr).astype('float32') # log transforming abundance
    ...
    bins = z_bin_x_stat(genome,False) # Bins of equal size, not spacing
    x_name = var[i].split('/')[-1] # ChIP name
    gc_abun = pd.DataFrame()
    gc_abun[x_name+'tss_abun_bin.u'] = bins[:,0]
    gc_abun[x_name+'tss_abun.u'] = bins[:,1]
    gc_abun[x_name+'tss_abun.s'] = bins[:,2]
    gc_abun[x_name+'tss_abun.m'] = bins[:,3]
    gc_abun[x_name+'tss_abun.X'] = bins[:,4]
    gc_abun.to_csv('./outputs/tss_abun/'+x_name+'tss_abun.500K.NAs.txt',sep='\t',index=False)
    gc_abun.dropna().to_csv('./outputs/tss_abun/'+x_name+'tss_abun.500K.txt',sep='\t',index=False)
