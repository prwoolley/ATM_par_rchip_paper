...
def z_bin_x_stat(array,bin_no,range_bin): # array[:,(z,x)]
    if bin_no == False:
        ...
        bin_no = len(array)//15_000_000 # Determining bin no
        bins = np.linspace(0,bin_no*15_000_000,bin_no+1) # creating equally SIZED bins
...
for i in range(2,8):
    ...
    gc_abun['bin_gc.u'] = bins[:,0]
    gc_abun[x_name+'tss_C_gc.u'] = bins[:,1]
    #gc_abun[x_name+'tss_C_gc.s'] = bins[:,2]
    #gc_abun[x_name+'tss_C_gc.m'] = bins[:,3]
    #gc_abun[x_name+'tss_C_gc.X'] = bins[:,4]
    gc_abun.to_csv('./outputs/tss_C_gc/'+x_name+'tss_C_gc.15mil.NAs.txt',sep='\t',index=False)
    gc_abun.dropna().to_csv('./outputs/tss_C_gc/'+x_name+'tss_C_gc.15mil.txt',sep='\t',index=False)
