...
def zw_bin_xy_stat_MODIFIED(array): # array[:,(z,x,w_y)]
    ...
    zbins = np.linspace(0,zbin_no*500_000,zbin_no+1) # Double check this line
    zbins = np.digitize(np.linspace(0,len(array),len(array)),zbins) # Double check this line
    out_array = []
...
for chrom in chrs:
    ...
    x_chr = np.load(var[3]+chrom+'.npy').astype('float32') # PAR
    y_chr = np.load(var[5]+chrom+'.npy').astype('float32') # RChIP
...
x_name = var[3].split('/')[-1] # ChIP name
y_name = var[5].split('/')[-1] # ChIP name
...
gc_abun.to_csv('par_x.rchip_wy.gc_z.dmso.figures.500K_25K_0.45GC.NAs.txt',sep='\t',index=False)
gc_abun.dropna().to_csv('par_x.rchip_wy.gc_z.dmso.figures.500K_25K_0.45GC.txt',sep='\t',index=False)
